...
2. Updates/error/success messages print here. You can see when the image has been loaded or any operations completed. An autosave operation happens every few minutes in the background so you don't lose your progress. 
...
3. The 3 image channels are shown on the bottom left. When you select the mask you can:
 ...
 5. You can toggle between 2D and 3D by pressing the square/cube button, or change viewing axis by pressing the third and fourht button. Press home to reset the view
 6. Scroll through the slices using this slider, or hold ctrl+mouse wheel. To zoom in/out use the mousewheel, and hold down space and drag to move. Ctrl + Z undoes any action.
 ...
 9. `FADE OUT`
 - If you image black to black, this works like INTERPOLATE, but on slice is all black (no signal)
 - e.g. layer a slice near the top and one that's fully out of focus. Whatever z you enter into `to z` will be set as all black.
 10. `SAVE`
...
# Advised Workflow
1. Decide on an image to curate in `Z:\Bel\IMAGES_FOR_VASCUMAP_RETRAINING\fluorescent_cells_tifs\3_channel_images_to_curate`. If you need me to breate base labels for a particular image, just let me know!
...
4. When you've pressed `Save final` (and seen the success message in the logger), check your image has been saved (you can see the "last edited" time in the file explorer).
- If it's not the current time, speak to Bel before you shut anything to ensure no progress is lost (it shouldn't be!)
